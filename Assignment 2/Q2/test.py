...
def cross_bilateral_filter(flash_image,no_flash_image,sigma_color,sigma_spatial):
    filtered_flash = bilateralfilter(flash_image, flash_image, sigma_spatial, sigma_color)
    filtered_no_flash = bilateralfilter(no_flash_image, no_flash_image, sigma_spatial, sigma_color)
    alpha = 0.4
    prolight_image = alpha * filtered_flash + (1 - alpha) * filtered_no_flash
    return prolight_image

flash_image = cv2.imread('./ultimate_test/2_b.jpg')
no_flash_image = cv2.imread('./ultimate_test/2_a.jpg')

sigma_color = 0.1*255
sigma_space = 0.1

bilateral_filtered_flash = cross_bilateral_filter(flash_image, no_flash_image, sigma_color, sigma_space)

cv2.imshow('prolight_image', bilateral_filtered_flash)
cv2.waitKey(0)
